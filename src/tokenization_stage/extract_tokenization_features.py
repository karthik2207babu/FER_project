...
def build_parser() -> argparse.ArgumentParser:
    ...
    default_output_dir = project_root / "outputs" / "tokenization_outputs"
...
def main() -> int:
    ...
    tokens_map_path = args.output_dir / f"{run_name}_tokens.pt"
    ...
    torch.save(tokens_cpu, tokens_map_path)

    # logs
    print(f"MTCNN status: {mtcnn_result.status}")
    print(f"Tokens shape: {tuple(tokens_cpu.shape)}")
